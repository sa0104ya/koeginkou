...
if (is_uploaded_file($_FILES["sample"]["tmp_name"])) {
    ...
    if (move_uploaded_file($_FILES["sample"]["tmp_name"], $updir.$filename)) {
		...
		mysql_query("INSERT INTO tsurugisanrokuon (filename,date,username) VALUES ('$filename','$date','$username')");

		//テーブルのカウントを取得
		$result = mysql_query("SELECT * FROM tsurugisanrokuon");
	...
	$re=mysql_query("SELECT * FROM tsurugisanrokuon ORDER BY date");
